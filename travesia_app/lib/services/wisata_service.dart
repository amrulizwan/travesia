...
class WisataService {
  ...
  Future<Wisata> addGalleryImage(String wisataId, String imagePath) async {
    try {
      final response = await _apiService.postMultipart('wisata/$wisataId/gallery', imagePath);
      if (response != null && response['data'] != null) {
        return Wisata.fromJson(response['data']);
      } else {
        throw Exception('Failed to parse add gallery image response or no data returned');
      }
    } catch (e) {
  ...
  Future<Wisata> updateWisata(String wisataId, Map<String, dynamic> wisataData) async {
    try {
  ...
  Future<Wisata> verifyGalleryImage(String wisataId, String gambarId, String status) async {
    try {
      final response = await _apiService.put('wisata/$wisataId/gallery/$gambarId/verify', {'status': status});
      if (response != null && response['data'] != null && response['data'] is Map<String, dynamic>) {
        ...
        throw Exception('Verification successful but no updated wisata data returned: ${response['message']}');
      } else {
        throw Exception('Failed to parse gallery verification response or no data returned');
      }
    } catch (e) {
      print('Error in verifyGalleryImage for wisata $wisataId, image $gambarId: $e');
      String rawMessage = e.toString();
      if (rawMessage.startsWith("Exception: ")) {
        rawMessage = rawMessage.substring("Exception: ".length);
      }
      List<String> parts = rawMessage.splitN(': ', 2);
      String userFriendlyMessage = 'Gagal verifikasi gambar galeri: $rawMessage';
      if (parts.length == 2) {
        String apiMessage = parts[1];
  ...
  Future<Wisata> assignPengelola(String wisataId, String newPengelolaId) async {
    try {
      final response = await _apiService.put('wisata/$wisataId/assign-pengelola', {'newPengelolaId': newPengelolaId});
      if (response != null && response['data'] != null) {
        return Wisata.fromJson(response['data']);
      } else {
        throw Exception('Failed to parse assign pengelola response or no data returned');
      }
    } catch (e) {
