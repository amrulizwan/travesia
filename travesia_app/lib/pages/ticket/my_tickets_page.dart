...
class _MyTicketsPageState extends State<MyTicketsPage> {
  ...
  void initState() {
    ...
    _authService = AuthService(_apiService); // AuthService needs ApiService
    _checkLoginAndFetchTickets();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Tiket Saya', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
  ...
  Widget _buildBody() {
      ...
      return Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(_errorMessage ?? 'Silakan login untuk melihat tiket Anda.', style: const TextStyle(fontSize: 16, color: Colors.redAccent)),
    ...
    if (_isLoading) { // Handles initial loading and loading during refresh
    ...
    return FutureBuilder<List<Ticket>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.waiting && _isLoading == false) {
          return const Center(child: CircularProgressIndicator(color: Colors.red));
        }
          ...
          return Center(
            child: Padding(
              ...
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Text('Anda belum memiliki tiket.', style: TextStyle(fontSize: 16)),
                  const SizedBox(height: 20),
                  ElevatedButton(
   ...
   Widget _buildErrorWidget(String errorMsg) {
    ...
    final displayError = errorMsg.startsWith("Exception: ") ? errorMsg.substring("Exception: ".length) : errorMsg;
    return Center(
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            ElevatedButton(
              onPressed: _isLoggedIn ? _fetchTickets : _checkLoginAndFetchTickets,
              style: ElevatedButton.styleFrom(backgroundColor: Colors.red),
  ...
  Widget _buildTicketCard(Ticket ticket) {
    ...
    } else if (ticket.items.isNotEmpty && ticket.items.first.idWisata.isNotEmpty) {
        // Potentially fetch wisata name by idWisata if needed, for now placeholder
        firstWisataName = 'Wisata ID: ${ticket.items.first.idWisata.substring(0,8)}...';
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Order ID: ${ticket.id}', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
            const SizedBox(height: 12),
            _buildDetailRow(Icons.event_available, 'Tgl Pembelian:', dateFormatter.format(ticket.tanggalPembelian)),
            _buildDetailRow(Icons.payment, 'Status:', ticket.statusPembayaran.toUpperCase(),
              valueColor: ticket.statusPembayaran.toLowerCase() == 'paid' ? Colors.green[700] : Colors.orange[700]),
            if(ticket.kodePembayaran != null && ticket.kodePembayaran!.isNotEmpty)
              _buildDetailRow(Icons.qr_code, 'Kode Pembayaran:', ticket.kodePembayaran!),
            if(ticket.tanggalKedaluwarsaPembayaran != null)
               _buildDetailRow(Icons.timer_off_outlined, 'Bayar Sebelum:', dateFormatter.format(ticket.tanggalKedaluwarsaPembayaran!)),
            const SizedBox(height: 12),
            Text('Detail Item:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            const SizedBox(height: 6),
            ...ticket.items.map((item) => _buildPurchasedItemDetail(item, currencyFormatter)).toList(),
            const Divider(height: 20, thickness: 1),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Total Harga:', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
                Text(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              value,
              style: TextStyle(fontSize: 14, color: valueColor ?? Colors.black87, fontWeight: valueColor != null ? FontWeight.bold : FontWeight.normal),
  ...
  Widget _buildPurchasedItemDetail(PurchasedItem item, NumberFormat formatter) {
    ...
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Expanded(child: Text('$itemName (x${item.jumlah})', style: const TextStyle(fontSize: 14))),
          Text(formatter.format(item.subtotal), style: TextStyle(fontSize: 14, color: Colors.grey[800])),
        ],
      ),
    );
  }
